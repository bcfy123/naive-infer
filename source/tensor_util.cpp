...
namespace naive_infer {
  std::tuple<sftensor, sftensor> TensorBroadcast(const sftensor& tensor1,
    ...
    if (tensor1->shapes() == tensor2->shapes()) {
      ...
      if (tensor2->rows()==1&&tensor2->cols()==1) {
        sftensor new_tensor = TensorCreate(tensor2->channels(),
          tensor1->rows(), tensor1->cols());
        CHECK(tensor2->size()==tensor2->size());
        for (uint32_t i=0; i<tensor2->channels(); i++) {
          new_tensor->slice(i).fill(tensor2->index(i));
        }
        ...
        LOG(FATAL) << "Broadcast shapes are not compatible!";
        return {tensor1, tensor2};
      }
    }
  }
  ...
  std::shared_ptr<Tensor<float>> TensorPadding(const std::shared_ptr<Tensor<float>> &tensor,
    ...
    for (uint32_t channel = 0; channel < channels; ++channel) {
      ...
      const uint32_t in_channel_cols = in_channel.n_cols;
      const uint32_t in_channel_rows = in_channel.n_rows;

      for (uint32_t w = 0; w < in_channel_cols; ++w) {
        float* output_channel_ptr =
          const_cast<float*>(output_channel.colptr(w + pad_cols1));
        const float* in_channel_ptr = in_channel.colptr(w);
        for (uint32_t h = 0; h < in_channel_rows; ++h) {
        ...
        for (uint32_t h = 0; h < pad_rows2; ++h) {
          *(output_channel_ptr + in_channel_rows + pad_rows1 + h) =
              padding_value;
        }
      }
      ...
      for (uint32_t w = 0; w < pad_cols1; ++w) {
        ...
        for (uint32_t h = 0; h < in_channel_rows + pad_rows1 + pad_rows2; ++h) {
          *(output_channel_ptr + h) = padding_value;
        }
      }
      ...
      for (uint32_t w = 0; w < pad_cols2; ++w) {
        float* output_channel_ptr = const_cast<float*>(
            output_channel.colptr(pad_cols1 + w + in_channel_cols));
        for (uint32_t h = 0; h < in_channel_rows + pad_rows1 + pad_rows2; ++h) {
          *(output_channel_ptr + h) = padding_value;
        }
      }
  ...
  bool TensorIsSame(const std::shared_ptr<Tensor<float>> &a, const std::shared_ptr<Tensor<float>> &b, float threshold) {
    CHECK(a != nullptr);
    CHECK(b != nullptr);
    if (a->shapes() != b->shapes()) {
      return false;
    }
    // arma::approx_equal parameters: comparison method and tolerance threshold.
  ...
  void TensorElementAdd(const std::shared_ptr<Tensor<float>> &tensor1, const std::shared_ptr<Tensor<float>> &tensor2,
  ...
  void TensorElementMultiply(const std::shared_ptr<Tensor<float>> &tensor1,
    const std::shared_ptr<Tensor<float>> &tensor2, const std::shared_ptr<Tensor<float>> &output_tensor) {
    CHECK(tensor1 != nullptr && tensor2 != nullptr && output_tensor != nullptr);
    if (tensor1->shapes() == tensor2->shapes()) {
      CHECK(tensor1->shapes() == output_tensor->shapes());
      output_tensor->set_data(tensor1->data() % tensor2->data());
