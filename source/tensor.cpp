...
namespace naive_infer {
  ...
  void Tensor<float>::Padding(const std::vector<uint32_t> &pads,
    ...
    // 2. 计算填充后新尺寸
    uint32_t new_rows = ori_rows + pad_rows1 + pad_rows2;
    uint32_t new_cols = ori_cols + pad_cols1 + pad_cols2;

    // 3. 创建填充后的cube，全部初始化为填充值
    arma::fcube new_data(new_rows, new_cols, channels, arma::fill::value(padding_value));
    // 4. 逐通道拷贝原数据到新cube的中间位置
    for (uint32_t c = 0; c < channels; ++c) {
      // 原通道矩阵
      const arma::fmat &ori_mat = this->slice(c);
      // 目标通道矩阵
      arma::fmat &dst_mat = new_data.slice(c);

      // 把原图拷贝到：行起始pad_rows1，列起始pad_cols1 的位置
      dst_mat.submat(pad_rows1, pad_cols1, pad_rows1 + ori_rows - 1, pad_cols1 + ori_cols - 1) = ori_mat;
      // 5. 替换原有数据
      this->data_ = std::move(new_data);
      // 6. 更新 raw_shapes_ 原始维度
      if (this->raw_shapes_.size() == 1) {
        // 一维张量 [N]，填充后变成二维，raw_shapes改为新的宽
  ...
  void Tensor<float>::Flatten(bool row_major) {
    ...
    // 2. 取出所有元素，按指定序展开成一维vector
    std::vector<float> flat_data = this->values(row_major);
    // 3. 重建底层cube：1通道，1行，total_elem列
    this->data_ = arma::fcube(1, total_elem, 1);
    // 4. 把展开的数据写入cube
    this->Fill(flat_data, row_major);
    // 5. 更新原始维度 raw_shapes_ 为一维 [总长度]
    this->raw_shapes_ = {total_elem};
  }
  ...
  // TODO 检查一下，是否有问题，不用引用
  std::shared_ptr<Tensor<float>>& operator-=(std::shared_ptr<Tensor<float>>& tensor, const float value) {
